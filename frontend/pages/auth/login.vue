...
const handleSubmit = async () => {
  ...
  if (form.password.trim().length < 8) {
    error.value = t('login.errorPasswordShort');
    return;
  }
  ...
  try {
    ...
    successMessage.value = t('login.loginSuccess', { name: user.display_name });
    await router.push("/");
      ...
      <section class="right-panel">
        <div class="form-wrapper">
          ...
          <div class="form-header">
            <h1 class="form-title">{{ $t('login.welcomeBack') }}</h1>
            <p class="form-subtitle">{{ $t('login.subtitle') }}</p>
          </div>

          <form class="form" @submit.prevent="handleSubmit">
            <div class="field-label">{{ $t('login.email') }}</div>
            ...
            <div class="form-meta">
              ...
              <NuxtLink to="/auth/forgot-password" class="forgot-link">{{ $t('login.forgotPassword') }}</NuxtLink>
            </div>
            ...
            <button type="button" class="google-btn">
              <LucideIcon name="Chrome" class="google-icon" aria-hidden="true" :stroke-width="2" />
              <span>{{ $t('login.googleLogin') }}</span>
            </button>

            <p class="signup-hint">
              {{ $t('login.noAccount') }}
              <NuxtLink to="/auth/register" class="signup-link">{{ $t('login.createNow') }}</NuxtLink>
            </p>
          </form>
        </div>
      </section>
