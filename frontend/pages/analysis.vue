...
const inputText = ref("从这个视频中提取完整的文本脚本，包括对话、旁白以及所有文字字幕：");
...
const confirmLinkModal = () => {
  ...
  if (!selectedUrls.value.includes(parsed.toString())) {
    selectedUrls.value.push(parsed.toString());
  }
  closeLinkModal();
};

const clearSelectedSource = (urlToRemove?: string | null) => {
  if (urlToRemove) {
...
const canSend = computed(() => {
  ...
  return (selectedFile.value || selectedUrls.value.length > 0) && inputText.value.trim().length >= 2;
});
...
const handleSend = async () => {
  ...
  try {
    ...
    if (selectedFile.value) {
      ...
      formData.append("title", `分析脚本 · ${selectedFile.value.name}`);
...
<template>
  <div class="flex-1 flex flex-col h-full relative bg-[#ffffff] dark:bg-[#121212]">
    ...
    <!-- Bottom Input Area (Fixed to bottom of main) -->
    <div class="absolute bottom-0 left-0 right-0 p-4 lg:px-[120px] bg-gradient-to-t from-white via-white dark:from-[#121212] dark:via-[#121212] to-transparent pointer-events-none pb-8">
      <div class="max-w-[800px] mx-auto pointer-events-auto">
        ...
        <div class="relative rounded-3xl bg-white dark:bg-[#1f1f1f] border border-[#e5e5e5] dark:border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_30px_rgba(0,0,0,0.3)] transition-all overflow-visible focus-within:ring-2 focus-within:ring-blue-100 dark:focus-within:ring-blue-500/20 focus-within:border-blue-300 dark:focus-within:border-blue-500/50">
          ...
          <!-- Source Tags -->
          <div v-if="selectedFile || selectedUrls.length > 0" class="px-4 pt-4 pb-1 flex flex-wrap gap-2">
            <div v-if="selectedFile" class="inline-flex items-center gap-2.5 rounded-2xl border border-[#ececec] bg-white px-3 py-2 shadow-[0_4px_12px_rgba(15,23,42,0.05)] max-w-[320px]">
              ...
              <div class="flex-1 min-w-0 overflow-hidden">
                ...
                <div class="text-[13px] text-[#9ca3af] truncate leading-tight mt-0.5">本地文件</div>
              </div>
            ...
            <div v-for="url in selectedUrls" :key="url" class="inline-flex items-center gap-2.5 rounded-2xl border border-[#ececec] bg-white px-3 py-2 shadow-[0_4px_12px_rgba(15,23,42,0.05)] max-w-[320px]">
              <div class="flex items-center justify-center w-9 h-9 rounded-full bg-[#efefef] text-[#3f3f46] shrink-0">
                <LucideIcon name="Link" class="w-4 h-4" :stroke-width="1.8" />
              </div>
              <div class="flex-1 min-w-0 overflow-hidden">
                <div class="text-[15px] font-semibold text-[#1a1a1a] leading-tight">{{ url }}</div>
                <div class="text-[13px] text-[#9ca3af] truncate leading-tight mt-0.5">链接</div>
              </div>
              <button @click.stop.prevent="clearSelectedSource(url)" class="flex items-center justify-center w-8 h-8 rounded-full text-[#9ca3af] hover:bg-[#f3f4f6] hover:text-[#6b7280] transition-colors shrink-0 z-10 relative">
                <LucideIcon name="X" class="w-5 h-5" :stroke-width="2" />
              </button>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between px-3 py-2 relative z-[100] border-t border-transparent">
            <div class="flex items-center gap-1.5 relative flex-wrap">
              ...
              <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[13px] font-medium border border-blue-100 dark:border-blue-500/20 shadow-sm cursor-default">
                <LucideIcon name="Captions" class="w-4 h-4 ml-0.5" />
                <span>分析脚本</span>
              </div>
              ...
              <!-- Attach Buttons -->
              <button @click="handleUploadClick" :disabled="selectedUrls.length > 0" class="flex items-center gap-1.5 px-2.5 py-1.5 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 rounded-full transition-colors text-[13px] disabled:opacity-50 disabled:cursor-not-allowed">
                <LucideIcon name="Paperclip" class="w-[18px] h-[18px]" stroke-width="1.8" />
                <span>上传视频</span>
              </button>
              <button @click="handleAddLinkClick" :disabled="selectedFile !== null" class="flex items-center gap-1.5 px-2.5 py-1.5 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 rounded-full transition-colors text-[13px] disabled:opacity-50 disabled:cursor-not-allowed">
                <LucideIcon name="Link" class="w-[18px] h-[18px]" stroke-width="1.8" />
                <span>添加链接</span>
              </button>
            </div>
    ...
    <input ref="fileInputRef" type="file" class="hidden" :accept="sourceVideoAccept" @change="handleFileChange" />
    <InputPromptModal
      :open="showLinkModal"
      title="添加链接"
      description="粘贴一个视频或网页链接进行分析"
      :value="linkDraft"
